...
async function getModeStats(userId) {
    ...
    for (const stat of data || []) {
        map.set(stat.frontier_mode, stat);
    }
    return map;
}
/**
 ...
 */
export async function calculateModeEntropy(userId) {
 ...
 */
export async function detectModeCollapse(userId, threshold = 0.6) {
 ...
 */
export async function selectFrontierMode(userId, pressure, config = DEFAULT_CONFIG) {
 ...
 * Record mode usage
 */
export async function recordModeUsage(userId, mode) {
 ...
 */
export async function updateModeSuccessRate(userId, mode, approved) {
 ...
 */
export async function initializeModeStats(userId) {
