 ...
 */
export async function calculateFAS(userId, idea, frontierBlocks) {
    // 1. Structural novelty: Has this domain pair been explored before?
    const domainPairNovelty = await calculateDomainPairNovelty(userId, idea.domain_pair);
    // 2. Conceptual distance: How far from existing idea clusters?
    const conceptualDistance = await calculateConceptualDistance(userId, idea.embedding || []);
    // 3. Tractability leap: Did we solve something previously hard?
    const tractabilityLeap = await calculateTractabilityLeap(userId, idea.domain_pair, idea.tractability_score || 0);
    // 4. Surprise factor: Was approval unexpected?
    const surpriseFactor = await calculateSurpriseFactor(userId, idea.domain_pair, idea.frontier_mode);
    // Weighted combination
    const overall = domainPairNovelty * 0.3 +
 ...
 */
async function calculateDomainPairNovelty(userId, domainPair) {
 ...
 */
async function calculateConceptualDistance(userId, embedding) {
 ...
 * 3. Tractability leap
 */
async function calculateTractabilityLeap(userId, domainPair, tractabilityScore) {
    ...
    const overlappingIdeas = data.filter((idea) => {
        const overlap = idea.domain_pair &&
            (idea.domain_pair.includes(domainPair[0]) ||
                idea.domain_pair.includes(domainPair[1]));
        return overlap;
    });
    if (overlappingIdeas.length === 0)
        return 0;
    // Calculate average tractability of prior attempts
    const avgPriorTractability = overlappingIdeas.reduce((sum, idea) => sum + (idea.tractability_score || 0), 0) / overlappingIdeas.length;
 ...
 */
async function calculateSurpriseFactor(userId, domainPair, frontierMode) {
 ...
 */
export async function createFrontierBlock(userId, idea, fas, abstractPattern) {
