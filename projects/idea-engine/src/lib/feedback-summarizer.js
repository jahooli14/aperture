 ...
 */
export async function getFeedbackWindow(userId, daysBack = 21 // 3 weeks
) {
    const end = new Date();
 ...
 */
export async function summarizeRejections(rejections) {
 ...
 */
export async function summarizeApprovals(approvals) {
    if (approvals.length === 0) {
        return 'No approved ideas yet.';
    }
    // Group by domain pair and mode
    const domainPairs = approvals.reduce((acc, a) => {
    ...
    const modes = approvals.reduce((acc, a) => {
        if (!acc[a.frontier_mode])
            acc[a.frontier_mode] = 0;
        acc[a.frontier_mode]++;
        return acc;
    }, {});
 ...
 */
export async function createFeedbackSummary(userId, windowDays = 21) {
 ...
 */
export async function getLatestFeedbackSummary(userId) {
    const { data, error } = await supabase
        .from('ie_feedback_summaries')
        .select('*')
        .eq('user_id', userId)
