 ...
 */
function calculateDomainDistance(domainA, domainB) {
 ...
 */
async function getDomainPairStats(userId) {
 ...
 */
export async function recordDomainPairGeneration(userId, domainA, domainB) {
    const [a, b] = [domainA, domainB].sort(); // Always sort alphabetically
    const { error } = await supabase.from('ie_domain_pairs').upsert({
        user_id: userId,
        domain_a: a,
        domain_b: b,
        times_generated: supabase.raw('times_generated + 1'),
 ...
 */
export async function sampleDomainPair(userId, config = DEFAULT_CONFIG, rejectionPatterns = []) {
    ...
    const mediumDistancePairs = allPairs.filter((p) => p.distance >= 0.3 && p.distance <= 0.7);
 ...
 */
export async function getNeglectedDomainPairs(userId, thresholdDays = 30, limit = 5) {
    ...
    return (data || []).map((row) => [row.domain_a, row.domain_b]);
}
/**
 * Initialize domain pairs in the database (run once)
 */
export async function initializeDomainPairs(userId) {
    const allDomains = domains.domains;
    const pairs = [];
    for (let i = 0; i < allDomains.length; i++) {
