...
import { Clock, ExternalLink, Archive, Trash2, WifiOff, Copy, Share2, Edit, Download, CheckCircle, MoreVertical, Loader2 } from 'lucide-react'
...
export const ArticleCard = React.memo(function ArticleCard({ article, onClick }: ArticleCardProps) {
  ...
  // The card used to carry its own sparked/off/make strip, writing
  // "reaction:"-prefixed tags. Nothing ever read them — the idea generator
  // reads metadata.reaction on LIST items, not tags on articles — so it was
  // three buttons that looked like they did something and didn't. Keeping or
  // rejecting an article is the end-of-article verdict, which is the one
  // answer that actually reaches the corpus.
          ...
          <div className={`flex-1 flex flex-col items-end justify-between ${article.thumbnail_url ? 'h-16' : 'min-h-[40px]'}`}>
            <div className="flex items-center gap-1">
              ...
              {isContentFullyCached ? (
                ...
                <div className="p-1 rounded-lg text-brand-text-secondary" style={{ background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.25)' }} title="Partially cached">
