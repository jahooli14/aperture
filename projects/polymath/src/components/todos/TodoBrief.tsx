...
import { Brain, X, Plus, ChevronDown, ChevronUp } from 'lucide-react'
        ...
        >
          ...
          {loading && (
            <div className="flex items-center gap-2 py-4">
              ...
              <span className="text-sm" style={{ color: "var(--brand-primary)" }}>Thinking about your day...</span>
            </div>
          )}
            ...
            >
                ...
                <div className="space-y-1.5 mb-3">
                  {brief.plan.map((item, i) => (
                    <div key={item.todo_id} className="flex gap-2 items-start">
                      <span className="text-[10px] font-bold mt-0.5 flex-shrink-0 w-4 text-center" style={{ color: "var(--brand-primary)" }}>
                        {i + 1}
                      </span>
              ...
              {brief.nudges.length > 0 && (
                <div className="border-t border-brand-border pt-3 space-y-2">
                  <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "var(--brand-primary)" }}>
                    Worth actioning
                  </span>
                  {brief.nudges.map((nudge, i) => (
                    <div key={i} className="flex items-start gap-2 group">
                      <p className="flex-1 text-xs leading-relaxed" style={{ color: "var(--brand-primary)" }}>
                        {nudge.text}
                      </p>
