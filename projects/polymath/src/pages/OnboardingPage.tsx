            ...
            <div className="p-6" style={{ background: 'var(--brand-glass-bg)', backdropFilter: 'blur(12px)', borderRadius: 'var(--premium-radius-lg)' }}>
              ...
              <div className="flex flex-wrap gap-2">
                {analysis.capabilities.map((cap, i) => (
                  <span key={i} className="px-3 py-1 rounded-full text-sm font-medium" style={{ backgroundColor: 'rgba(59, 130, 246, 0.15)', color: 'var(--brand-primary)' }}>
                    {cap}
                  </span>
                ))}
              </div>
              ...
              {analysis.graph_preview.nodes.map((node, i) => (
                <div key={node.id} className="flex flex-col items-center">
                  <div className="w-24 h-24 rounded-full flex items-center justify-center text-sm font-medium text-center p-2"
                    style={{ backgroundColor: 'rgba(59, 130, 246, 0.15)', color: 'var(--brand-primary)' }}>
                    {node.label}
                  </div>
                </div>
              ))}
