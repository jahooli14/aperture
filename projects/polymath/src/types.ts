...
export interface TriageInfo {
  category: 'task_update' | 'new_thought' | 'reading_lead' | 'new_project_idea' | 'list_item' | 'annoyance' | 'taste_signal'
  project_id?: string
  confidence: number
...
export interface Project {
  ...
  type?: string | null // Project category - optional, defaults to 'creative'
  ...
  // Execution rebuild (SPEC.md) — supersedes is_priority as the source of
  // truth for what's being worked on. is_priority stays for the old home
  // surface until it's retired.
  state?: 'live' | 'on-deck' | 'mull' | 'harvested'
  mvs_minutes?: number | null // minimum viable session, learned from real sessions
...
/** One checkpoint in a project's arc -- every time the task list ran out,
 *  what judgeFinishLine actually said against the stated finish line.
 *  Mirrors api/_lib/project-milestones.ts's Milestone. */
...
export interface ProjectMetadata {
  ...
  target_date?: string // ISO date. Optional, user-volunteered only -- never asked for, same as end_goal itself.
  milestones?: ProjectMilestone[] // The arc: every time the list ran out, what judgeFinishLine said (api/_lib/project-milestones.ts)
