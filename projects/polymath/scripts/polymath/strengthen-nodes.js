 ...
 */
async function getRecentCommits(sinceHours = CONFIG.LOOKBACK_HOURS) {
    console.log(`📜 Fetching git commits from last ${sinceHours} hours...`);
    const sinceDate = new Date(Date.now() - sinceHours * 60 * 60 * 1000);
    const sinceISO = sinceDate.toISOString();
    try {
        // Get commit hashes
        const { stdout: hashesOutput } = await execAsync(`git log --since="${sinceISO}" --format=%H`, { cwd: CONFIG.APERTURE_PATH });
        ...
        for (const hash of hashes) {
            // Get commit info
            const { stdout: infoOutput } = await execAsync(`git show --format=%an%n%aI%n%s --no-patch ${hash}`, { cwd: CONFIG.APERTURE_PATH });
            const [author, date, message] = infoOutput.trim().split('\n');
            // Get changed files
            const { stdout: filesOutput } = await execAsync(`git diff-tree --no-commit-id --name-only -r ${hash}`, { cwd: CONFIG.APERTURE_PATH });
 ...
 */
async function getCapabilityId(capabilityName) {
 ...
 */
async function incrementNodeStrength(nodeType, nodeId, increment) {
 ...
 */
export async function strengthenNodes(sinceHours = CONFIG.LOOKBACK_HOURS) {
    ...
    for (const commit of commits) {
        ...
        const projectUpdate = await incrementNodeStrength('project', projectId, CONFIG.PROJECT_INCREMENT);
        if (projectUpdate) {
        ...
        for (const capabilityName of project.capabilities) {
            ...
            const capUpdate = await incrementNodeStrength('capability', capabilityId, CONFIG.CAPABILITY_INCREMENT);
            if (capUpdate) {
