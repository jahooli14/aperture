 ...
 * Clears all existing data and loads fresh demo seed data
 * Run: npx tsx scripts/reset-with-demo.ts
 */
...
async function resetWithDemo() {
  console.log('🗑️  Clearing existing data...\n')
  // 1. Clear existing data (in order of foreign key dependencies)
  console.log('Deleting project suggestions...')
  const { error: sugError } = await supabase
    .from('project_suggestions')
    .delete()
    .neq('id', '00000000-0000-0000-0000-000000000000') // Delete all
  if (sugError) console.error('Error deleting suggestions:', sugError)
  else console.log('✅ Cleared project_suggestions')
  console.log('Deleting projects...')
  const { error: projError } = await supabase
    .from('projects')
    .delete()
    .neq('id', '00000000-0000-0000-0000-000000000000') // Delete all
  if (projError) console.error('Error deleting projects:', projError)
  ...
  else console.log('✅ Cleared memories')
  console.log('Deleting capabilities...')
  const { error: capError } = await supabase
    .from('capabilities')
    .delete()
    .neq('id', '00000000-0000-0000-0000-000000000000') // Delete all
  if (capError) console.error('Error deleting capabilities:', capError)
  else console.log('✅ Cleared capabilities\n')
  // 2. Load demo data
  ...
  const { data: insertedMemories, error: memInsertError } = await supabase
    .from('memories')
    .insert(memories)
    .select()
  if (memInsertError) {
    console.error('❌ Error seeding memories:', memInsertError)
    return
  }
  ...
  const { data: insertedCaps, error: capInsertError } = await supabase
    .from('capabilities')
    .upsert(capabilities, { onConflict: 'name' })
    .select()
  if (capInsertError) {
    console.error('❌ Error seeding capabilities:', capInsertError)
    return
  }
  ...
  const projects = [
    {
      user_id: USER_ID,      title: 'Standing Desk Build',
    ...
    {
      user_id: USER_ID,      title: 'Morning Meditation Routine',
  ...
  const { error: projInsertError } = await supabase
    .from('projects')
    .insert(projects)
  if (projInsertError) {
    console.error('❌ Error seeding projects:', projInsertError)
    return
  }
