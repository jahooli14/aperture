...
function getRoadTypeFromStrength(strength) {
    if (strength >= 11)
        return 'highway';
    if (strength >= 6)
        return 'main';
 ...
 */
function extractTopicsFromText(text, maxTopics = 3) {
    if (!text)
        return [];
 ...
 */
function kMeansClustering(embeddings, k, maxIterations = 5 // Reduced from 10 for better performance
) {
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
        ...
        for (let i = 0; i < actualK; i++) {
            ...
            const diff = Math.sqrt(centroids[i].reduce((sum, val, idx) => sum + Math.pow(val - newCentroid[idx], 2), 0));
            if (diff > 0.001)
                changed = true;
 ...
 */
function forceDirectedLayout(cities, roads, clusterCenters, iterations = 25 // Reduced from 50 for better performance
) {
 ...
 */
export async function generateInitialMap(userId) {
 ...
 */
function getRegionColor(clusterId) {
    const colors = [
