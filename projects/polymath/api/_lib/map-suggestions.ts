...
export async function generateDoorSuggestions(
  ...
  for (let i = 0; i < currentMap.cities.length; i++) {
    for (let j = i + 1; j < currentMap.cities.length; j++) {
      ...
      const roadExists = currentMap.roads.some((r: Road) =>
      ...
      const sharedItems = cityA.itemIds.filter((id: string) => cityB.itemIds.includes(id))
  ...
  const allAssignedIds = new Set(currentMap.cities.flatMap((c: City) => c.itemIds))
  ...
  unassignedItems.forEach(item => {
    const topics = item.entities?.topics || item.metadata?.capabilities?.map((c: any) => typeof c === 'string' ? c : c.name) || item.tags || []
  ...
  topicClusters.forEach((items, topic) => {
    if (items.length >= 3 && !currentMap.cities.some((c: City) => c.name === topic)) {
      // Position in open space (offset from center of map)
      const avgX = currentMap.cities.reduce((sum: number, c: City) => sum + c.position.x, 0) / currentMap.cities.length || 500
      const avgY = currentMap.cities.reduce((sum: number, c: City) => sum + c.position.y, 0) / currentMap.cities.length || 500
  ...
  const largeCities = currentMap.cities.filter((c: City) => c.population >= 10)
