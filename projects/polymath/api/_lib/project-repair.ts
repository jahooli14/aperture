...
export async function ensureProjectHasTasks(projectId: string, userId: string): Promise<boolean> {
    ...
    if (tasks.length >= 1) return true // Changed from 3 to 1: Respect user's desire for a lean list.
...
export async function repairAllUserProjects(userId: string) {
    ...
    const emptyProjects = projects.filter(p => !p.metadata?.tasks || p.metadata.tasks.length < 1)
