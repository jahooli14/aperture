...
export async function sendDailyDigest(userId: string, ideas: Idea[]) {
  ...
  const html = approvedIdeas.length > 0
    ...
    : generateEmptyDigestHTML(progress);

  const subject = approvedIdeas.length > 0
    ? `🔬 Daily Idea Digest — ${approvedIdeas.length} new building block${approvedIdeas.length === 1 ? '' : 's'}`
    : `🔬 Daily Idea Digest — No new ideas today`;
...
function generateEmptyDigestHTML(progress: ProgressStats): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background: #ffffff;">
  <div style="max-width: 680px; margin: 0 auto; padding: 40px 20px;">
    <div style="margin-bottom: 32px;">
      ...
      <p style="margin: 0 0 8px 0; font-size: 16px; color: #64748b;">
        No new building blocks today
      </p>
      <p style="margin: 0; font-size: 14px; color: #94a3b8;">
        The engine ran but no ideas passed review today. This can happen if generation volume was low or the reviewer was extra selective. Check the stats below.
      </p>
    </div>
    ...
    <div style="margin-bottom: 32px; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px; color: white;">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
        ...
        <div>
          ...
          <div style="font-size: 13px; opacity: 0.9;">Reviewed Today</div>
        </div>
      </div>
    ...
    <div style="padding: 24px; background: #fef3c7; border-radius: 8px; border-left: 4px solid #f59e0b; margin-bottom: 24px;">
      <p style="margin: 0; font-size: 14px; color: #92400e; line-height: 1.6;">
        <strong>Heads up:</strong> ${progress.today.generated === 0
          ? 'No ideas were generated in the last 24 hours. The generation workflow may not be running — check GitHub Actions.'
          : progress.today.reviewed === 0
            ? 'Ideas were generated but none were reviewed. The review workflow may not have run — check GitHub Actions.'
            : 'Ideas were generated and reviewed, but none passed the bar today. The system is working as expected.'}
      </p>
    </div>
