...
async function getTimelinePatterns() {
    ...
    const monthlySideHustle = new Map();
...
- If they've abandoned projects, suggest smaller scope`;
    ...
    const opportunitiesWithMeta = opportunities.map((opp, i) => ({
        id: `opp-${Date.now()}-${i}`,
        ...opp,
...
async function getShadowProjects() {
    ...
    const { data: articles } = await supabase
        ...
        .eq('user_id', userId)
        .gte('created_at', cutoff.toISOString());
    if (!memories?.length && !articles?.length)
...
async function getSmartSuggestion() {
    ...
    const hotStreakProjects = projects.filter(p => p.status === 'active' &&
        p.priority === true);
    ...
    if (timeOfDay === 'morning' && !isWeekend) {
        const freshProjects = projects.filter(p => p.status === 'active' &&
    ...
    if (timeOfDay === 'evening') {
        const quickProjects = projects.filter(p => p.status === 'active' &&
    ...
    if (isWeekend) {
        const creativeProjects = projects.filter(p => p.status === 'active' &&
...
function shouldSuggestCapture(memories) {
 ...
 */
async function getInspiration(excludeProjectIds) {
    ...
    const stats = {
        ...
        embeddings: {
            ...
            overall: {
                ...
                coverage: Math.round(((projectsWithEmbeddings || 0) + (thoughtsWithEmbeddings || 0) + (articlesWithEmbeddings || 0)) /
