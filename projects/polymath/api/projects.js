...
function calculateStaleness(project) {
    const lastActive = new Date(project.last_active);
    const now = new Date();
    const daysSinceActive = daysBetween(lastActive, now);
...
function calculateFreshness(project) {
...
function calculateAlignment(project, context) {
    let score = 0;
    ...
    const timeMatches = ((context.available_time === 'quick' && estimatedTime <= 30) ||
    ...
    const requirementsMet = requirements.every((req) => context.available_context.includes(req));
    if (requirementsMet)
        score += 5;
    return score;
}
function calculateUnlockBonus(project) {
    if (project.recently_unblocked)
        return 5;
    const blockers = project.blockers || [];
    if (blockers.length === 0 && project.had_blockers_before)
        return 3;
    return 0;
}
function scoreProject(project, context) {
...
function selectDailyQueue(scores) {
    const queue = [];
    ...
    const needsAttention = scores
        .filter(s => s.breakdown.staleness >= 15 &&
    ...
    const freshEnergy = scores
        .filter(s => s.breakdown.freshness >= 10 &&
        !queue.find(q => q.project_id === s.project_id))
 ...
 */
async function handleListSuggestions(req, res, supabase, userId) {
    try {
        // Query parameters
        const { status = 'pending', limit = '20', offset = '0', include_rated = 'false' } = req.query;
        let query = supabase
        ...
        const enrichedSuggestions = await Promise.all((data || []).map(async (suggestion) => {
 ...
 */
async function handleRateSuggestion(req, res, id, supabase, userId) {
    if (!id) {
 ...
 */
async function handleBuildFromSuggestion(req, res, id, supabase, userId) {
    if (!id) {
        return res.status(400).json({ error: 'Suggestion ID required' });
    }
 ...
 */
async function generateProjectEmbeddingAndConnect(projectId, title, description, userId) {
    const supabase = getSupabaseClient();
    try {
        ...
        for (const candidate of candidates) {
            if (candidate.similarity > 0.85) {
                ...
                const created = await createConnection('project', projectId, candidate.type, candidate.id, 'relates_to', 'ai', `${Math.round(candidate.similarity * 100)}% semantic match`);
                if (created) {
 ...
 */
async function findRelatedItemsForProject(projectId, userId, projectEmbedding, projectContent) {
    const supabase = getSupabaseClient();
    const candidates = [];
 ...
 */
async function createConnection(sourceType, sourceId, targetType, targetId, connectionType, createdBy, reasoning) {
    const supabase = getSupabaseClient();
 ...
 */
async function handleGetBedtimePrompts(req, res, supabase, userId) {
    try {
...
async function handleGenerateBedtimePrompts(req, res, supabase, userId) {
    try {
        const { generateBedtimePrompts } = await import('../lib/bedtime-ideas.js');
        const prompts = await generateBedtimePrompts(userId);
        return res.status(201).json({
...
async function handleGenerateCatalystPrompts(req, res, supabase, userId) {
    try {
...
async function handleMarkBedtimeViewed(req, res, supabase, userId) {
    try {
 ...
 */
async function incrementCapabilityStrength(capabilityId, increment, supabase) {
 ...
 */
async function penalizeCombination(capabilityIds, penalty, supabase) {
    try {
