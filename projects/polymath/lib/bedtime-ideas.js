 ...
 */
export async function generateCatalystPrompts(inputs, userId) {
...
export async function generateBedtimePrompts(userId) {
    ...
    const prompts = await generatePromptsWithAI(recentArticles, recentMemories, activeProjects, currentInterests, oldInsights, { hasRichInput, hasBlockedProjects, hasNoProjects }, performance);
 ...
 */
async function getRecentArticles(userId, days) {
 ...
 */
async function getRecentMemories(userId, days) {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - days);
 ...
 */
async function getCurrentInterests(userId) {
 ...
 */
async function getOldInsights(userId, daysAgo) {
    const cutoff = new Date();
 ...
 */
function detectConnections(articles, memories, projects) {
    const connections = [];
    ...
    const memoryEntities = new Set(memories.flatMap(m => (m.entities || []).map((e) => e.name)));
    // Find overlaps
    const themeOverlaps = [...articleTags].filter(tag => memoryThemes.has(tag) || memoryEntities.has(tag));
    if (themeOverlaps.length > 0) {
        connections.push(`Cross-pollination: "${themeOverlaps.slice(0, 3).join('", "')}" appears in both your reading and your thoughts`);
    }
    ...
    for (const project of dormantProjects) {
        const projectKeywords = (project.title + ' ' + (project.description || ''))
            .toLowerCase()
            .split(/\s+/);
        const hasThemeMatch = [...memoryThemes].some(theme => projectKeywords.some(word => word.includes(theme.toLowerCase())));
        if (hasThemeMatch) {
            connections.push(`Dormant project "${project.title}" relates to your recent thinking`);
        }
    }
 ...
 */
async function generateCatalystPromptsWithAI(inputs, userId) {
 ...
 */
async function generatePromptsWithAI(recentArticles, recentMemories, activeProjects, currentInterests, oldInsights, context, performance) {
    const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
 ...
 */
async function storePrompts(userId, prompts) {
 ...
 */
async function getPromptPerformance(userId) {
