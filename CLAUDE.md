...
- **Sparks** (`mull-generator.ts` + `mull-prompts.ts` + `mull.ts` + `mull-corpus.ts`, `bake` cron resource, daily 08:00 UTC) — baked overnight, stands for four days so it can sit unanswered until the answer turns up on a walk. **Rebuilt Sept 2026 around one idea: a revelation is something they already know and have never said, and it only shows when two or more things they captured apart are put side by side.** The previous channel grounded every question in ONE row and policed taste with regexes, so all it could do was read a note back with a question mark on the end — live, the day of the rebuild: *"Your outline centers on an unseen person gathering fragments of someone's life inside a house. What is 'a piece they need to create'?"* Honest, and empty.
  ...
  - **Judge** — one Flash call (`low` thinking) reads each survivor against its FULL evidence rows and scores revelation / truth / specific / answerable, with a verdict and a reason. Ships only on its own "ship" with truth ≥ 7, revelation ≥ 7, answerable ≥ 6 (`judgeShips`), ranked with revelation counting double. If the judge call fails, only the first honest candidate ships — never a queue of unjudged ones.
...
**The next move — one move, written at the end of the last session** (`api/_lib/next-move.ts`, `next-move-store.ts`; SPEC.md → "Where the move comes from"). In creative work the next step comes out of the last one, so a list written up front is out of date by item two and never gets used. Each project keeps ONE move in `metadata.next_move`, rewritten at close-out (in parallel with the debrief) from the note just left — where you stopped, what's next, what's bugging you — plus what was ticked. Opening the app is then instant: the move is on the card, Go starts it. There is no session planner, no window question, no step list shown as orders.
- **Three openings**: a brand-new project may get a *fork* ("the pole, or the wires?") whose answer (kept as a `constraint` fragment) writes the first move; a going project's move comes out of the last note; a project a month away gets a way-back-in move. `resource=move` (`get` / `feedback` / `say` / `answer` / `set`) is the only route; the nightly cron fills missing or month-stale moves for the few projects most likely to be opened.
- **Rejections stick** (`metadata.move_feedback`): "too big" twice makes the writer go smaller; a turned-down move is never offered again (checked, not just asked for).
- **Tasks are a log.** A ticked move lands in `metadata.tasks` as done work with its "Done when" stripped. The old step list (`task-spine.ts` — still written at creation and for a repeating project's next cycle) is background the writer reads as "where it was heading", folded away on the project page (`NextMovePanel`). `order` and `progress_note` still hold for it (`task-order.ts`).
- **Deleted, don't recreate**: `session-shaper.ts`, `session-prebake.ts`, `session-split.ts`, `session-topup.ts`, `session-ready.ts`, `session-briefing.ts`, `session-budget.ts`, `session-confidence.ts`, `session-gap.ts`, `finish-line.ts`, `offlinePlan.ts`, the `shape` and `replan` resources and "Plan what comes next". They built and trimmed a list; the move replaced the list.
...
- **AI thinking cost**: Flash-Lite is a *thinking* model — thinking tokens bill as output ($1.50/1M). Mechanical classify/extract/score calls pass a capped thinking level via `thinkingFragment()` in `api/_lib/gemini-thinking.ts`; creative synthesis stays on the model default. `GEMINI_THINKING_LEVEL` (Vercel env: `minimal`/`low`/`medium`/`high`) globally overrides every wired call — dial it up if output quality dips, down to cut cost. Keep new mechanical calls capped; never cap creative idea/prose generation. The spark channel (`mull-generator.ts`) drafts and judges on Flash at `low` (no hidden thinking on this model) inside a 10s budget — never Pro (too slow, too expensive, barely better).
- **Type + contrast** (`theme.css`, `design-tokens.css`): Playfair (`--brand-font-display`, Tailwind `font-display`) is for page titles ≥ 24px only; everything read at body size uses Literata (`--brand-font-serif`, `font-serif`). Nothing smaller than 11px, uppercase labels tracked ≤ 0.14em, and never fade text below ~0.6 opacity — make it quieter with size or the `--brand-text-muted` colour instead. Faded italic Playfair on the haze is what made the app hard to read. Style is **neon glass, cyan**: `.glass-card`/`.neon-edge` draw a lit gradient edge, `.neon-breathe` a slow 9s glow (opacity-only, cheap), `.neon-button` the one lit action per card. The mull question is the one ethereal text (`.mull-text`): italic Literata, pale cyan-white, each sentence surfacing out of a blur and settling sharp. Every animation stops under `prefers-reduced-motion`.
