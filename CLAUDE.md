...
- `NULL` — undecided, and here the rule is **did you open it** (`read_at`). An article you opened counts; one that only ever sat in the feed does not. The verdict-or-nothing version of this rule read well and in practice meant a live corpus of **198 articles with zero of them counting**: the buttons are at the *end* of an article, so most reads never tap one, and the whole reading input sat dark while five of the mull channel's six gatherers returned nothing. What the rule exists to exclude is the twenty headlines that arrive overnight and are never touched — still excluded, because they were never opened. Choosing one out of the feed and reading it is a real signal; it just isn't as strong as saying so, which is what `corpusWeight` is for (2 for `good`, 1 for opened-or-hand-saved). The legacy **hand-saved** carve-out (no `rss` tag) stands as it always did.
  > `read_at` only, never `status`. Status looks like it says this and doesn't: the right-swipe on New reads sets `'reading'` to mean "I want this in my list" (its own comment says it hasn't been read), and the left-swipe on Saved reads sets `'archived'` without opening anything. Both are filing gestures. `read_at` is stamped in exactly one place — the reader loading the article — which is also where an opened article now earns its embedding, since an eligible row with no embedding is invisible to every search that gates on one. Every query feeding `isCorpusEligible` must select `read_at` (`mull-subjects.ts`, `synthesis.ts`, `project-ideas/gather.ts`, `reading.ts`) or the column's absence reads as "never opened" and silently drops the row.
