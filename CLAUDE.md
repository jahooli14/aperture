...
- **Sparks** (`spark-generator.ts`, `bake` cron resource, daily 08:00 UTC) — one per day, baked overnight so it's instant and offline-available, not generated on open. Rotates through *types* (noticing, transferred constraint, your own unfinished thought played back, a contradiction between two things you've said, a scale jump, a material fact, and "outside reach" — something from your reading/RSS, not your own corpus, so the system doesn't just recombine you forever). Not a task — answer by voice in 30 seconds or let it expire silently. Usually **not** the live project on purpose. Rotating the type and the project isn't enough on its own — three days running came back about water and infinity, each a different type about a different project — so `spark-echo.ts` (pure, unit-tested) also rotates the *subject*: the last few sparks go into every prompt verbatim as an avoid-list (the only half that catches "waves" for "ripples"), and the finished spark is dropped if it reuses a motif that has already recurred or shares two distinctive words with one recent spark. A dropped spark isn't a lost day — the bake loop tries the next type.
