...
Live, on a 34-project corpus: *"They gave up on 'Custom t-shirts' in January 2026. 6 months later they started 'Create custom t-shirts for friends' from scratch, as a separate project, with no reference to the first one."* and *"On 3 January 2026 they started 3 projects in one sitting — and every one of them is dead."* Guards that matter: `harvested` projects are finished, never failures, so they can never appear as one; a batch where anything survived is a productive afternoon, not a question; and a near-duplicate made the same week is a mistake, not a return (`RESTART_MIN_GAP_DAYS`). The prompt is told not to console and not to scold — it is a fact they had no way of seeing, not a telling-off.
