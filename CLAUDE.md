...
- **Embeddings** (`api/_lib/gemini-embeddings.ts`, `gemini-embedding-001` at 768 dims via MRL). Three things worth knowing, all verified against the [live docs](https://ai.google.dev/gemini-api/docs/embeddings) (Sept 2026):
  ...
  > **Unresolved, and deliberately not done without a decision:** we pass no `taskType`, so corpus and query vectors share one undifferentiated space when the mull channel's retrieval is asymmetric (a blind-spot question against a corpus of notes — that's `RETRIEVAL_QUERY` vs `RETRIEVAL_DOCUMENT`). Separately, `gemini-embedding-2` is GA (April 2026): 8192-token input, auto-normalizes truncated dims, and takes the task as a prompt instruction instead of a parameter. **Either change rewrites the vector space, so it is a flag day: every stored embedding must be rebuilt in one go or old and new rows silently stop being comparable.** That is cheap here (~360 rows) but it is a real decision, and mixing spaces degrades quietly rather than failing. Also latent: the ivfflat indexes are `lists = 100` on tables of 34–198 rows, which is far too many lists for the row count (pgvector wants ~rows/1000, and `probes` defaults to 1) — harmless while Postgres seq-scans tables this small, a silent recall collapse once it stops.
