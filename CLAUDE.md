...
**A third shape: a haul nobody came back to** (`findUntouchedHaul`). On 10 January ten lines went onto a list in one sitting — *"be excellent to each other"*, *"dancing on water in life's late hours"* — and every one is still untouched. What makes it worth asking is invisible to the list: the user's very first capture, months earlier, was a project to collect beautiful sentences. They did the project in one afternoon, in the wrong container, and never called it that. The app can't know the second half; it can put the first half in front of them, which is the trick. Guards: one of them being picked up makes it a list working as intended, not a pile; four days qualify on this corpus and only the biggest ships, because four questions about "you saved some things once" is one question and three repeats.
