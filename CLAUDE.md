...
- **Sparks** (`mull-generator.ts` + `mull.ts`, `bake` cron resource, daily 08:00 UTC) — one per day, baked overnight so it's instant and offline-available, not generated on open. It stands for four days, because the point is that it gets to sit unanswered until the answer turns up on a walk. **One mechanism, not a menu of question shapes.** Take one subject (a project, usually — momentum-weighted with a swerve — or a recent note, or an article that earned its place), name the **blind spot** it takes for granted, rewrite that blind spot as a plain human question with *none of the subject's own vocabulary left in it*, and embed-search the whole corpus with that. Whatever comes back is the connector, and it was chosen *by* the blind spot, so there is nothing left to invent. **A question with no project on either side is not the product.** The search already returns project rows alongside memories and articles for every blind spot, but similarity alone doesn't prefer one kind over another — an unfiled thought about a football match paired exactly as readily with another stray note as with an actual project, and produced self-contained trivia with nothing to build. So a subject already anchored to a project (a project subject itself, or a joint/pair whose captures trace to one) is left alone, but everything else — an unfiled thought, an article, a project-less pair — **requires** a project connector in band (`selectConnectors`'s `requireProject`): no fallback, an empty result is correct silence rather than settling for a note. Joints are the one exception (`preferProject`, soft): a project-less joint is the mechanism for surfacing a *new* project ("a thing you keep saying and have never made is the shortest path there is to 'oh, I should make that'"), so it's allowed to connect to something that isn't a project yet either. **A buried project doesn't count as unlocking one.** `match_projects` filters nothing on state or status — only that the project has an embedding — so with a project connector now often required, a project the user sent to the graveyard was exactly as eligible as a live one. The app's usual guard, `.neq('state', 'harvested')`, doesn't catch it either: "send to graveyard" (`projects.ts`) sets `status: 'abandoned'` and leaves `state` at `'mull'` — only completion sets `state: 'harvested'`. `isGraveyarded` (`mull.ts`) checks both, in JS rather than a query filter (`.neq()` drops NULL rows), and is applied where a project can become either a subject or a connector. The vocabulary strip is the load-bearing step: search "how is replacing a character different from developing one" and you get the chapter outline back — the subject restated; search "what it's like when someone you know becomes a different person" and you reach the note about your dad's garden. Retrieval is **banded** (`CONNECTOR_FLOOR`/`CONNECTOR_CEILING`): above the ceiling it's the note restated, below the floor it's noise, and inside the band anything sharing two distinctive words with the subject is dropped however well it scores — relevance from the vector, distance from the vocabulary. **The ceiling scales with how much vocabulary the subject actually has** (`connectorCeiling`): the two guards are meant to work together, but the best subjects are short and plain — "it only works if it's one take" is three distinctive words and nine stopwords — so `sharesDomain` blocks nothing for them, including the note that is that idea in different clothes. When one guard is absent the other tightens. The draft is then gated by rules, not taste (`rejectionReason`): the note's own words must survive into the question — either the model's `quote` really is in the note, or the question itself carries a verbatim run of it (`longestSharedRun`), because the model mis-reports what it used far more often than it invents, and throwing a sound question away for a clerical slip just produces an empty slot; it ends in a question, sixty words max, and it may not contain "which mirrors" / "this connects to" / "both are about" — explaining the link is the tell that there wasn't one. **The corpus is a time series, not a bag of rows** (`corpus-time.ts`, pure + unit-tested; subjects assembled in `mull-subjects.ts`). Every selector this channel had was recency-bound — the newest 150 fragments for joint mining, 45 days for notes and articles, momentum weighting for projects — so a corpus with years in it was only ever asked what happened lately, and the years are where the convictions are. Now the subject is a **temporal shape**: a true, dated statement computed from timestamps, with the captures that prove it. `long_unfinished` (said since March 2023, never became a project — the strongest one there is), `return` (silent 14 months, back last week, where the silence must be out of character for *that* timeline or a slow conviction reads as a comeback), `conviction`, `went_quiet` (a rhythm that stopped in a nameable month), `burst` (one week two years ago, never again), `drift` (early vocabulary vs late), and `simultaneity` — two things captured days apart under different projects and never joined since, which is **purely temporal and therefore invisible to vector search**, since embeddings can only return things that resemble each other. **Span beats count everywhere**: five fragments from one Tuesday is one thought, so `mine-joints` now reads the whole corpus and drops any cluster under `MIN_SPAN_DAYS`. List items get a time dimension too (a want held a year is not a mood). **Capture time is not thought time**, which is the confound under all of it: a project going quiet in August might be abandoned, or a baby arrived and everything went quiet at once. `buildActivityBaseline` is the missing denominator — a month where the whole corpus fell silent explains any one project falling silent, and a month where everything spiked explains any one burst. So `went_quiet`, `burst` and `return` all decline when a global lull accounts for the silence. No guess about the cause, just the arithmetic. **Every span threshold is sized to the corpus that exists** (`scaleToCorpus`). All of them — 45-day recurrences, 400-day convictions, 150-day silences — were written for a corpus with years in it, which is what the channel was designed against and not what it usually runs on. Asking "has this been held for 400 days" of a corpus four months old isn't strict, it's *unanswerable*: no arrangement of the data can satisfy it, so the gatherer returns zero and the channel goes quiet for a reason that has nothing to do with the user's thinking. A live run had 72 fragments, every one of them embedded and clusterable, and **zero** recurring themes. A threshold is only a fair question once the corpus covers **twice** it, so below that it scales down in step, floored (`MIN_SPAN_FLOOR_DAYS` and friends) so "said twice on Tuesday" still can't count as a recurrence. A mature corpus is untouched — three years is past twice every ideal, so it gets the constants unchanged. Scaling against a fixed *fraction* of the corpus instead was tried and is wrong: an eighth of three years quietly loosens conviction to 137 days, which is a quality regression wearing a tuning fix's clothes. **Six subjects go into the one blind-spot call now, capped at three of any one kind** — was three and two, and it was throwing away most of what the corpus had already computed for free. A live run typically gathers ~24 candidates (joints 12, projects 9, pairs 2, long-held 1), and `rankPairs` downstream already caps what gets *drafted* at `PAIRS_TO_DRAFT` (4) regardless of how many subjects feed it, preferring distinct subjects first — so 3 slots was never a cost control, it was discarding 21 of 24 candidates before ranking ever saw them. Measured, not argued: doubling the slots reached six genuinely distinct subjects (*Vivid dreams book*, *Fun to be had*, a drift joint, *A single note on paper*, two simultaneity pairs) where the old cap kept re-serving the same one or two joints a burst of "ask me something else" taps had already exhausted, because with only 3 slots and a 2-per-kind cap a corpus with 12 joints in it could only ever put 2 up per call — whichever two were strongest, which is by definition whichever had just been asked and declined. The two model calls this run makes do not change; the wider net costs a slightly bigger (still capped-thinking) blind-spot call and three more Postgres searches, and Postgres and embeddings are the cheap half of this pipeline. Capped, deduped by kind only" — also deduping by shape collapsed ten project subjects into one when they all classified the same way, and one subject means one blind spot, one pair, one draft, so a single gate rejection at the end produces nothing. Three exist precisely so two can fail — joints carry several shapes and outrank everything, so deduping by shape alone let three joints take every slot and ask three questions about the user's own recurring thoughts. **The cap fills rather than starves**: once the capped pass is done, any slot still empty is filled ignoring the cap. Stopping one kind crowding out the others is the right job while there *are* others — when four of the five gatherers return nothing, which is the ordinary case on a young corpus, the cap stops being a variety rule and becomes a throttle on the only thing that works (ten project subjects available, two taken, third slot left empty, so one gate rejection at the end produced silence). An empty slot protects nothing, and a third subject of a kind already used is still a different project, a different blind spot and a different draft. Reading keeps a slot whenever any exists, since an article has no temporal shape and on strength alone would never once be chosen; `unfiled` (a thought old enough to have been filed and never was) makes thoughts a subject rather than only a connector. The model never chooses the shape and never supplies the fact — it's handed one and asked to write the question, and the prompt requires it to use the real date, because a date is the one part the user can't argue with and the one part they had no way of seeing from inside.

**A question the user waved away is not history.** The echo gate stops the channel asking the same thing twice, and it turned poisonous: every question in the window had been generated from the broken corpus, several of them the specific bad questions this week's work exists to prevent. The first healthy run wrote four good drafts and threw three away as echoes — one for sharing "arsenal", "winning" and "streak" with the football question that *was* the bug. The channel was protecting its own mistakes from correction. `fetchRecentSparkTexts` now skips dismissed rows (in JS — the column is nullable), and `20260915_retire_prefix_sparks.sql` retires everything baked before the corpus was repaired. Same shape as the `forgotten` mistake below, arriving by another door.

**The note has to reach the QUESTION, not just the setup.** `draftQuality` → `noteReachesQuestion` asks whether three consecutive words of the material survive into the sentence with the question mark in it. Three live runs scored `0.00, 0.00, 0.00, 0.00` — the measure is not broken, it is reporting total failure, and the failure is the one you can see in the output: *"You wrote that a man drops off the internet to rediscover himself, becoming a postman. Does the knowledge base keep running in the background, or does the user have to walk away?"* The note is quoted and then abandoned; the question is in the model's vocabulary and could have been asked with no note at all. The prompt now demands their phrase inside the question (*"Does Aperture only work if you drop off the internet first?"*) and that anything **running out** is named — a clock or a price gets answered where a neutral observation gets admired and forgotten.

**A binary is banned by shape, not by wording — unless the stake splits.** With a healthy corpus and the quote landing correctly, the channel still shipped *"Does Pupils trace how he grows up, or does it stay in the nursery?"* and *"Does Tame impala synth sessions run on footwork, or does it stay on the synth?"* The prompt had banned this from the first draft, as the phrasing *"X, and also Y — which is it?"* — which the model simply never used. `offersAChoice` catches the shape instead: an opening auxiliary **and** an explicit alternative, both halves required so that "what would it take to finish it, or to admit it is finished?" still ships. But some of the best questions here *are* either/ors, and the stake tells them apart: *"Do you have it now, or did you miss the work?"* pays off as "he writes the last line this week, **or** admits he wanted the hours" — two branches, two actions. A two-sided question behind a single-outcome stake had decorative sides. `stakeSplits` is the test, and `mull-examples.test.ts` caught the over-broad first version by failing on the channel's own teaching example.

**The strongest material is arithmetic over the project table, and no search can reach it** (`api/_lib/project-shapes.ts`, pure + unit-tested). A thing given up on and quietly started again six months later; a day when three projects were opened and every one of them died. Nothing here *resembles* anything, so vector retrieval is structurally blind to all of it — and the pipeline used to lose even the facts it did compute: a temporal shape became a blind-spot prompt, became a search query, paired on similarity, and dropped the fact on the floor. Orbit was the one path that carried its claim to the draft, and orbit was the one path producing defensible questions. These do the same, and sort ahead of orbit, because a claim about what someone *did* beats a claim about what a vector says.

Live, on a 34-project corpus: *"They gave up on 'Custom t-shirts' in January 2026. 6 months later they started 'Create custom t-shirts for friends' from scratch, as a separate project, with no reference to the first one."* and *"On 3 January 2026 they started 3 projects in one sitting — and every one of them is dead."* `RESTART_SIM` is 0.82, measured not chosen: project-to-project similarity runs p50 0.52, p90 0.58, p99 0.72, the one true restart scores 0.84, and the next pair down is 0.81 — *"A single note on paper"* and *"Paint one wood block"*, two different projects sharing a register. A claim about what someone **did** has to sit above the corpus's ceiling for coincidence, not just above its average. **A fact must name everything a question will reasonably say about it.** The restart fact first read *"6 months later"*, so a draft would write "and started it again in June" — and `unsupportedSpecifics` rejected it, correctly by its own rules, because June appeared nowhere in the evidence. Every restart question would have died on a date the fact could have supplied. The fact now names both months. A gate that can only see what it is given has to be given it, and this was caught by running the real gates against the real fact before shipping, not after. Other guards: `harvested` projects are finished, never failures, so they can never appear as one; a batch where anything survived is a productive afternoon, not a question; and a near-duplicate made the same week is a mistake, not a return (`RESTART_MIN_GAP_DAYS`). The prompt is told not to console and not to scold — it is a fact they had no way of seeing, not a telling-off.

**Measured on the real corpus with the real model**, not argued about. Running `bakeMull` locally against production Supabase and Gemini costs about a third of a penny a run, and one run answers questions a week of reasoning could not. What it showed: the restart shape ships a good question in five runs out of five, `draftQuality` went from `0.00, 0.00, 0.00, 0.00` across three earlier runs to `1.00` on the best draft, and two fixes were found that no unit test would have caught. The model pads the setup with scraps of whatever field it was handed — *"...started them again from scratch in June WITH CREATIVE LOGO T-SHIRTS"*, a tail lifted wholesale from `description` that says nothing — so the prompt now demands two sentences, fact then question, with that exact line as the anti-example. And **a draft scoring 0.00 no longer ships when a better one exists**: a zero means the note never reached the question, so *"Which country starts continent by continent?"* is not a weaker question but a broken one, and there is no reason to spend a four-day slot on it. A lone zero still ships — the slot is otherwise empty, and the repeated lesson here is that a mediocre question beats nothing.

Live output, after: *"You gave up on custom t-shirts in January. In June you started them again from scratch. Which name goes on the first tag?"* Known leak: one run produced *"Are you making X, or are you just running the same January project twice?"* — a binary that passed through the `stakeSplits` exemption. The exemption is right in principle and too loose in practice.

**A third shape: a haul nobody came back to** (`findUntouchedHaul`). On 10 January ten lines went onto a list in one sitting — *"be excellent to each other"*, *"dancing on water in life's late hours"* — and every one is still untouched. What makes it worth asking is invisible to the list: the user's very first capture, months earlier, was a project to collect beautiful sentences. They did the project in one afternoon, in the wrong container, and never called it that. The app can't know the second half; it can put the first half in front of them, which is the trick. A question with no project carries `projectId: **null**`, never `''` — it lands in `sparks.project_id`, a uuid column Postgres refuses an empty string for, and a run's rows are inserted together, so one project-less question would have failed the whole bake. Guards: one of them being picked up makes it a list working as intended, not a pile; four days qualify on this corpus and only the biggest ships, because four questions about "you saved some things once" is one question and three repeats.

**Ordinals are not inventions.** English writes dates as ordinals and these computed facts write them as cardinals, so every question that mentioned the day was rejected for making it up — two drafts in one run died on "3rd" and "10th" against a fact reading *"On 10 January 2026…"*. Same shape as the missing month above: a gate can only see what it is given, and here it was given the number and shown the ordinal. `ordinalSupported` matches a one- or two-digit ordinal against the bare number, and nothing else.

**`stakeSplits` asks whether the second branch says anything new** (tested in `mull.test.ts`). The word "or" alone was enough, and one run in five proved it a hole: *"Are you making creative logo t-shirts for friends, or are you just running the same January project twice?"* shipped behind a stake that said "or" and meant nothing by it. Two attempted fixes were both wrong. `/\beither way\b/` passed the stake **unconditionally** — the whole gate handed away on a phrase meaning *regardless of which branch*, which is the opposite of splitting and exactly what the model writes when both branches land in the same place. Requiring four words a side reads as strictness and is really a length check, and the channel's own teaching examples fail it: *"The shed gets racking or gets emptied"* and *"The plot gets planted this spring or handed back"* are real splits whose second side is **two words**, because English drops the repeated subject in the second branch. What separates the three real splits on record from a decorative one is that the second side **names something the first did not** — racking/emptied, planted/handed back, writes/admits — so both sides must carry content and the side after "or" must contribute a content word of its own, compared on crude stems (*"He decides, or he does not decide"* is one outcome said twice, and an exact match reads `decide` as new next to `decides`). Neither `stakeSplits` nor `offersAChoice` had a single test before this, which is how the leak survived; both halves are covered now, accepts and rejects.

**The stake is stored** (`sparks.stake`, `20260916_spark_stake.sql`). It is the gates' own evidence and it was being computed, used, and thrown away — so when a binary leaks, the one thing that explains why it passed is gone. A live one did: *"Does Aperture pull the raw thoughts straight from your notes, or wait until they are finished?"* stood for a day with nothing on the row to diagnose it. Same lesson as `unsupportedSpecifics` and the restart fact, pointed backwards: a gate that can only see what it is given has to be given it, and so does anyone reading the gate afterwards. `insertSparks` drops the column on `42703` like the embedding writers drop `embedded_at`, because a diagnostic field is never worth failing the run it describes, and `npm run mull:try` prints it under each question. **A real split stake does not guarantee a real choice, though** — one leak shipped behind *"The first batch gets printed or the project goes back in the drawer"*, which splits by any reading. That case is now observable rather than fixed; don't tighten it on a guess, since the gates have thrown away three good questions in five before and that fails silently as an empty slot.

**Run it locally before arguing about it** — `npm run mull:try` (`scripts/try-mull.ts`). The real channel against the real corpus and the real model, writing nothing, for about a third of a penny. A week went into reasoning about why every draft scored `0.00`; one run of this settled it in five minutes by printing what the model actually wrote. It takes the user id from the corpus rather than a constant, because a hardcoded default printed "0 rows" for every table and looked exactly like a broken corpus.

**A joint that fits no sharper shape is still a joint** (`recurringFallback`). `classifyTimeline`'s bars are built for raw timelines, and a joint has *already* proved recurrence — the miner only writes one for a cluster spanning `MIN_SPAN_DAYS`+. Making it clear a shape bar as well counts that evidence twice, and the arithmetic leaves a dead zone exactly where a young corpus lives: at 321 days, `conviction` wants 160 and `went_quiet` wants 120 days of *silence*, so anything recurring across two or three months and still warm matched nothing. Live: three joints in the table, zero joint subjects — the channel's strongest kind, discarded on a technicality. The shapes add colour; their absence subtracts none of the dated fact underneath. Same class, two more places: `long_held` (365 days) and `unfiled` (120) are scaled too, since "wanted for a year" is unanswerable on a corpus 321 days old (`long-held-candidates: 0 rows` was exactly that).

**The question the model is asked not to repeat is settled before the calls, not after.** The avoid-list works on the question's *wording* at draft time — after the blind-spot call and all nine searches are paid for — so a run whose subjects are the projects a standing question already covers spends both model calls to be told, correctly, that it has nothing new: `4 pairs came back, 4 declined by the model`. `sparks.project_id` records which project each question was about, exactly, so those subjects are demoted at selection instead. Demoted, never dropped: with a handful of candidates, excluding them trades a repeat for an empty slot, and a repeat still has the gates in front of it.

**But `project_id` can only demote a subject that HAS a project, and a joint routinely doesn't.** A project-less joint is the mechanism for surfacing a *new* project, so it's explicitly allowed to have no project on either side — and that means nothing about it was ever demotable: the strongest joint in the corpus could be drafted, shipped, read, and dismissed, and the next run would rank it exactly as high as if it had never been asked. Live: an `untouched_haul` subject (a January list, project-less by the same design as joints) scored `1.00` on `draftQuality` in one run, then died in `echoesRecent` for repeating a question about the same list asked minutes earlier in the same sitting — a repeat the demotion step should have caught before either model call, not after both. `sparks.subject_id` / `subject_kind` (`20260916_spark_subject.sql`) name the subject directly, whichever kind it is, and `fetchRecentSparkSubjectIds` demotes by that identity alongside `recentProjectIds` — closing the gap for exactly the kinds (joints, project-less pairs) the project-only version structurally could not reach. Still only catches what *shipped*: a subject drafted and declined by the model, or drafted and gate-rejected, writes no row and leaves no trace here either — that gap is real, and is why the slot widening above exists as well, rather than relying on demotion alone to spread load across a burst of rerolls in one sitting.

**An article connector quotes the article.** `match_reading` returns `excerpt`, capped at 100 characters by the ingest for the card UI, and the connector is the *only* text the model may quote — so the gate's demand that the note's own words survive into the question was being made of a teaser that barely exists. One live question read "while reading about the Switch to mobile version": nav chrome, because that was the whole blurb. The real text is one lookup away, the same way the graveyard check above already re-queries the ids `match_projects` returned.

**Simultaneity counts each thought once.** A filed thought appears twice in the capture list — as the memory and as the fragment pointing at it, both carrying the same date — so one co-occurrence between two projects became four pairings, and the guard asking for a pair that happened *exactly once* could never pass (`144 captures -> 276 pairs -> 0 unique`). The guard is right: the fact it protects is "never put them together since". It was being fed the same event repeatedly.

**Where the drafts go is traced too.** The parser required `spark` AND `quote` AND `stake`, so a missing quote dropped the draft before `rejectionReason` — the gate that exists to re-check the question itself when the model mis-reports what it used, which is the field it gets wrong most. Both reach the gates now, where a rejection is readable, and a *failed* draft call says so in the trace instead of only in the Vercel log, where it read identically to a model that had nothing to say.
...
**Two model calls per run, whatever happens, and a run usually feeds the channel for a week.** Model calls are the expense; Postgres and embeddings are close to free, so the work sits either side of them. Call one names the blind spot for *every* subject at once (a project, a recent note, an article) — three blind spots for the price of one, thinking capped to `low` since naming an assumption is closer to extraction than prose. Retrieval then runs three searches instead of one, which is the step most likely to come back empty and the one that costs nothing to repeat. Call two writes up the best **four** surviving pairs together (`rankPairs` — deterministic, no third call to choose), and the first two that clear the gates are kept.

**Depth before the quality bar, never instead of it.** The channel is a chain of narrowings, and every stage can independently return nothing, so the pass rates multiply: a production run went `10 candidate projects → 1 subject → 1 connector (out of 40 in band) → 1 draft → 1 gate rejection → nothing`. Every one of those narrowings was a choice, not a limit in the corpus. So `selectConnectors` returns the top few per blind spot rather than only the best, `rankPairs` tops up to `PAIRS_TO_DRAFT` with near-duplicate pairs as reserve, and distinctness is enforced on what **ships** rather than on what gets attempted. Same two model calls; four shots at the gates instead of one. Widening the funnel is the lever — never loosening the gates, which are the only thing keeping invention out. The second question is **banked**: written with a longer expiry so it sorts behind the standing one (`today` serves soonest-to-expire), then handed over with no cron run and no model call when the first is answered or runs out. `reroll-spark` checks the bank before generating, so "ask me something else" is usually free and instant. Shelf life is stamped on first sight, not on generation, or a banked question would arrive half spent. `spark-echo.ts` still rotates the *subject* (recent sparks go into the prompt verbatim as an avoid-list, and a spark reusing a recurring motif is dropped) — but only **`mull` sparks count as history**: a `forgotten` offer is "you set down <project> N months ago", so its distinctive words are a project *title*, and leaving it in made every real question about that project read as an echo of the nag. One run wrote four good questions and threw away all four. Silence is the common case and it's correct — most nights one of the four steps declines. **`bake?explain=1` says which one**, with its numbers (per-table row counts, per-gatherer counts, subjects and their shapes, each search's candidate count and best score against the band it had to fit, gate rejections). **Every query reports its error into the trace** (`noteQuery`) — `const { data } = await supabase…` makes a rejected query and an empty table the same value, which is how a column that does not exist presented as "the user has nothing to say", and writes nothing; a plain `bake` returns the same trace alongside `baked:false`. Silence is the designed outcome of four separate steps and they are indistinguishable from outside, so "the channel is quiet" was an unanswerable bug report without this. `mull-pipeline.test.ts` runs the whole channel against a fake corpus with a stubbed Gemini (five gatherers, nine searches, two calls), and its stub **rejects a select naming a column the fixture lacks**, the way PostgREST does — a stub that answers any column hid a `memories.project_id` that does not exist (fragments is the link table: `memory_id` + `project_id`) and read as an empty corpus for a day, and `mull-gates.test.ts` is a false-reject harness rather than a unit test: the gates once threw away three good questions in five, and that fails silently as an empty slot.
  > The nine rotating spark types (`noticing`, `transferred_constraint`, `contradiction`, `scale_jump`, `material_fact`, `outside_reach`, `gap`, `unfinished_thought`) and the answer-rate bandit that weighted them are **deleted** (`spark-generator.ts`, `spark-types.ts`). Each type picked its own pair and asked the model to bridge it, which is the machine that produces forced mashups — rotation made the collisions varied, not true. Don't recreate them. `session-gap.ts` lives on, but only where it always belonged: session shaping. `forgotten.ts` and its `ForgottenSlot` are **deleted too**. "You set down X, N months ago" is a fact about the calendar, not a reason to care — and because it was the *fallback*, it only ever appeared when the channel had found no insight, making it a card that by construction carried none. It held the slot for four days at a time and, being made of project titles, blocked every real question about those projects as an echo of itself. **Resurfacing a dormant project is still the channel's job, but it has to earn it**: `long_unfinished` and `return` do exactly that, with a dated fact and a question attached. An empty slot is the honest alternative and the home surface already renders nothing there. `today` also refuses to serve legacy `forgotten` rows, so the stored ones vanish without a migration.

**"Get more creative" is a second, explicit tier below the regular bar — never a silent one.** Widening the funnel (above) still leaves nights where nothing survives every gate, and the honest answer used to always be an empty slot. Now the reroll button offers one more tap once it's already come back with nothing: `generateMull`'s `loose` flag (threaded through `reroll-spark` → `retireAndRebake` → `bakeMull`), never reached by the nightly bake or by the first "ask me something else" tap. It relaxes taste, not honesty, and each relaxation is its own explicit opt-in rather than one global switch: subjects a recent question already covered stop being demoted (nothing left *but* recently-covered ground once you're here), a project-less connector is allowed where the strict pass requires one (`findConnectors`'s fallback, kept as a second pass rather than weakening `selectConnectors` itself — that function's no-fallback contract is right on its own terms), and `rejectionReason` stands down on the explainer-pattern check, the binary-choice/`stakeSplits` pairing, and `stakeIsHollow`. Grounding and `unsupportedSpecifics` are never negotiable in any tier — a lower bar changes how good the question has to be, not whether it's allowed to invent something.

**Below that, one tier deeper: `draftSolo` — no connector at all.** A corpus can be thin enough that even the loosened connector search comes back with nothing for every subject, and the honest answer used to stop there ("Really nothing to work with right now"). It doesn't anymore: when creative mode still ships zero questions, `draftSolo` hands the draft call one subject's own material — `Subject.block`, every fragment quoted — and asks it to find a plot twist already sitting *inside* that one thing, rather than a link to anything else. This is deliberately the exact move `Subject.line`'s own comment warns the regular draft call away from: handing over the whole block lets the model pick two of a project's own fragments and collide them, which is pair-first invention rebuilt inside a single subject. That used to be a bug (a live run produced "You wanted to map all 198 countries to a memory palace. Yet you left your painted coasters sitting for eleven months after writing down the Esqui ice saga" — three fragments of one project, collided, asking what any of it had to do with the rest). Here it's the point: there is nothing else left to supply the tension, so the block becomes the material on purpose. Grounding still applies exactly as elsewhere — the gates don't know or care whether the quoted material came from a connector or a block, so a claimed twist still has to be a real quote, not an invention. `mull-pipeline.test.ts` covers it end to end: a project-less thought with no connector anywhere in the corpus, even under creative relaxation, still ships a question — two model calls total (blind spot, then straight to the solo draft — `draftAll` never runs because there was nothing to send it).
...
- **Embeddings** (`api/_lib/gemini-embeddings.ts`, `gemini-embedding-001` at 768 dims via MRL). Three things worth knowing, all verified against the [live docs](https://ai.google.dev/gemini-api/docs/embeddings) (Sept 2026):
  ...
  - **One vector space, and it is the query one — measured, not chosen.** Retrieval here is asymmetric (a blind-spot question against a corpus of notes), so the textbook answer is `RETRIEVAL_DOCUMENT` for stored rows and `RETRIEVAL_QUERY` for searches. That answer assumes retrieval is the only thing the vectors are for, and here it is **one use out of eight**: the same `memories.embedding` column also decides which project a capture attaches to (`fragments.ts`), which projects are a restart of each other (`project-shapes.ts`), which notes cluster into a joint (`joints.ts`), and where a project's centre of mass sits (`orbit.ts`) — all document-against-document and symmetric. `RETRIEVAL_DOCUMENT` is trained to make a document findable *by a query*, not to hold documents apart from each other, and on this corpus it does the opposite. The whole corpus was rebuilt both ways and measured: attach margins p50 `0.0081` → `0.0164` and p90 `0.0407` → `0.0807`, and the headroom a restart needs above the corpus's ceiling for coincidence (project↔project `max − p99`) `0.043` → `0.121`. The ranking degrades with the numbers — in query space the woodwork note reaches *Paint one wood block* and the dream-door note reaches *Vivid dreams book*, two of the four pairs named above as the ones that stop being arguable, and in document space both drop out of the top six for vaguer pairs. So it is `CORPUS_TASK`, a named constant with the numbers attached, rather than a per-call parameter someone can set on one writer and quietly break the thresholds everywhere else. Passing no `taskType` at all is byte-identical to it (cosine 1.000000 against the live API), so this names what was already true. **Every measured constant in the codebase lives in this space** — `ATTACH_MARGIN`, `ORBIT_FLOOR`/`CEILING`/`RIVAL_MARGIN`, `CONNECTOR_FLOOR`/`CEILING`, `RESTART_SIM`, `RECURRENCE_SIM_THRESHOLD` — so changing it is a flag day for the vectors *and* a retune of all eight. That is what makes it a real decision and not a tuning knob: in document space `RESTART_SIM` 0.82 admitted 8 project pairs instead of 1, and `RECURRENCE_SIM_THRESHOLD` 0.72 admitted 1551 of 2850 note pairs — over half the corpus as one recurring theme. Neither fails; both quietly stop meaning anything.
...
Every query in the mull channel (`api/_lib/mull-subjects.ts`, `api/_lib/mull-generator.ts`, `api/_lib/joint-miner.ts`, `api/_lib/drift-runner.ts`) reports its own errors into the trace or console rather than letting `const { data } = await supabase…` read a rejection as an empty table — that exact bug cost a day (`memories.project_id` didn't exist) and then recurred twice more (a PostgREST embed on `memories(embedding)` that silently kept the joints table empty for months; `sparks.type` missing `'mull'` from its CHECK constraint, read by the UI as "couldn't reach the server"). `mull-subjects.ts`'s `longHeldSubject` retries once on a transient error (Gateway Timeout, ECONNRESET) — the one query-failure class worth retrying, scoped to where it was actually observed rather than wrapped speculatively around every query.
